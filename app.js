...
function toggleSlide(id) {
  ...
  if (thumb) thumb.style.transform = `translateX(${comp.state.pos === 1 ? 0 : 20}px)`;
   ...
   function handleDrag(e) {
     ...
     const rect = workspace.getBoundingClientRect();
     const coords = getPointerCoords(e);
     let nx = coords.clientX - dragOffset.x;
     let ny = coords.clientY - dragOffset.y;
     nx = Math.max(0, Math.min(nx, rect.width - 192));
   ...
   function updateWires() {
     ...
     wires.forEach((wire, index) => {
       ...
       const x1 = (rF.left+rF.width/2) - rect.left;
       const y1 = (rF.top+rF.height/2) - rect.top;
       const x2 = (rT.left+rT.width/2) - rect.left;
       const y2 = (rT.top+rT.height/2) - rect.top;
   ...
   function simulationTick() {
     ...
     if(mEl && tutorialGuides[currentTutorial]) mEl.innerHTML=tutorialGuides[currentTutorial].liveMetrics();
   }
