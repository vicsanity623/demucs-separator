...
class ImprovedCTRNN:
    ...
    def tick(self, dt, sensors, uncertainty=None, use_planning=False, precomputed_net_input=None):
        ...
        if len(self.voltage_history) > ATTENTION_WINDOW * 2: self.voltage_history = self.voltage_history[-ATTENTION_WINDOW:]
...
def crossover(b1, b2):
    ...
    child.weights = np.where(np.random.rand(*b1.weights.shape) < 0.5, b1.weights, b2.weights)
    child.biases = np.where(np.random.rand(*b1.biases.shape) < 0.5, b1.biases, b2.biases)
    child.time_constants = np.where(np.random.rand(*b1.time_constants.shape) < 0.5, b1.time_constants, b2.time_constants)
...
def deepseek_style_mutate(brain):
    ...
    nb.biases = np.clip(brain.biases * 0.99 + np.random.normal(0, 0.05, brain.biases.shape) * (np.random.rand(*brain.biases.shape) < 0.1), -1.5, 1.5)
    return nb