...
class Environment:
    def __init__(self, king_gen, max_health=150):
        ...
        self.food_positions = np.random.uniform(10.0, WORLD_SIZE-10.0, (self.num_food, 2))
        self.food_vels = np.random.uniform(-0.2, 0.2, (self.num_food, 2)) if self.gen > 200 else np.zeros((self.num_food, 2))
        self.poison_positions = np.random.uniform(10.0, WORLD_SIZE-10.0, (self.num_poison, 2))
    ...
    def update(self, motor_output, brain=None):
        ...
        hit_wall = (new_pos[0]<=1.0 or new_pos[0]>=WORLD_SIZE-1.0 or new_pos[1]<=1.0 or new_pos[1]>=WORLD_SIZE-1.0)
        ...
        if self.ticks > 0 and self.ticks % 1800 == 0:
            ...
            self.health = min(self.max_health, self.health + 100) # Healing reward for reproducing
        ...
        out_of_bounds = (self.poison_positions < 0) | (self.poison_positions > WORLD_SIZE)
        ...
        if num_poisoned > 0:
            ...
            self.poison_positions[poisoned_mask] = np.random.uniform(10.0, WORLD_SIZE-10.0, (num_poisoned, 2))
    ...
    while time.time() < deadline:
        ...
        for i in range(NUM_AGENTS):
            ...
            if not alive:
                ...
                score = (envs[i].ticks * 0.1) + (envs[i].food_count * 8000) + (envs[i].children_spawned * 50000) + center_bonus - (envs[i].wall_contact_count * 10)
